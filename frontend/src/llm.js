...
function LLM({ events, setEvents }) {
  const [messages, setMessages] = useState([{ role: "ai", text: 'Hello! I can help you book tickets. Try saying "Book 2 tickets for Tiger Football Game".' }]);
  const [input, setInput] = useState("");
  const [proposedBooking, setProposedBooking] = useState(null);
  const [listening, setListening] = useState(false);
  const [isOpen, setIsOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    recognition.onerror = () => { setListening(false); addMessage("ai", "Sorry, I had trouble hearing you."); };
  ...
  const addMessage = (role, text) => {
    setMessages(prev => [...prev, { role, text }]);
    if (role === "ai" && narratorEnabled && window.speechSynthesis) {
      const utterance = new SpeechSynthesisUtterance(text);
      window.speechSynthesis.speak(utterance);
    }
  };
  ...
  const toggleListening = () => {
    if (!recognitionRef.current) { addMessage("ai", "Voice input not supported."); return; }
    if (!listening) { setListening(true); recognitionRef.current.start(); addMessage("user", "[Listening...]"); }
  ...
  const handleSendMessage = async (messageText = input) => {
    ...
    try {
      const response = await axios.post(`${LLM_API}/api/llm/parse`, { text: textToSend });
      const data = response.data;
      ...
      if (data.intent === "book_tickets" && data.message) {
        ...
        if (data.events?.length > 0) {
          const eventList = data.events.map(e => `${e.event_name} on ${e.event_date} at ${e.event_location} (${e.event_tickets} tickets)`).join("\n");
          addMessage("ai", `Here are the events:\n${eventList}`);
        } else addMessage("ai", "No events found.");
      } else addMessage("ai", data.message || "How can I help you?");
    } catch (err) { addMessage("ai", "Sorry, I encountered an error."); console.error(err); }
    finally { setLoading(false); }
  };

  const handleConfirm = async () => {
    if (!proposedBooking) return;
    setLoading(true); addMessage("user", "Yes, confirm booking");

    try {
      const response = await axios.post(`${LLM_API}/api/llm/confirm`, proposedBooking);
      addMessage("ai", response.data.message || "Booking confirmed!");

      if (setEvents) {
        setEvents(prev => prev.map(ev => ev.event_name === proposedBooking.event
          ? { ...ev, event_tickets: Math.max(0, ev.event_tickets - proposedBooking.tickets) }
          : ev));
      }
      setProposedBooking(null);
    } catch (err) {
      console.error(err);
      addMessage("ai", err.response?.data?.error || "Failed to confirm booking.");
    } finally { setLoading(false); }
  };
  ...
  return (
    <>
      <button className={`chatbot-toggle ${isOpen ? "open" : ""}`} onClick={() => setIsOpen(!isOpen)} aria-label="Toggle AI assistant">{isOpen ? "×" : "AI"}</button>
      <div className={`chatbot-panel ${isOpen ? "open" : ""}`}>
        <div className="chatbot-header">
          <h3>AI Ticket Assistant</h3>
          <div className="narrator-toggle">
            <label>
              <input type="checkbox" checked={narratorEnabled} onChange={() => setNarratorEnabled(!narratorEnabled)} />
              Narrator
            </label>
          </div>
        ...
        <div className="chat-input-container">
          {proposedBooking && (
            <button className="confirm-btn" onClick={handleConfirm} disabled={loading}>Confirm Booking</button>
          )}
          ...
          <div className="input-row">
            ...
            <button className={`voice-btn ${listening ? "listening" : ""}`} onClick={toggleListening} disabled={loading}>{listening ? "Stop" : "Voice"}</button>
            <button className="send-btn" onClick={() => handleSendMessage()} disabled={loading || !input.trim()}>Send</button>
          </div>
        </div>
      </div>
    </>
