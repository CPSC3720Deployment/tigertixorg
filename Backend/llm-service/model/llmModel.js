...
/**
 * Decrement tickets for an event
 */
async function decrementTickets(event_id, tickets) {
  const res = await pool.query(
    `UPDATE Event 
     SET event_tickets = event_tickets - $1 
     WHERE event_id = $2 AND event_tickets >= $1
     RETURNING *`,
    [tickets, event_id]
  );
...
/**
 * Create booking record (simulate)
 */
async function createBooking(event_id, tickets) {
  return {
    booking_id: Math.floor(Math.random() * 100000),
    event_id,
    tickets,
