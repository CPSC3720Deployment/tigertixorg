 ...
 * Inserts a new event into the Event table.
 */
const createEvent = async (event_name, event_date, event_tickets, event_location) => {
    const eventQuery = `
        INSERT INTO Event (event_name, event_date, event_tickets, event_location)
        VALUES ($1, $2, $3, $4)
        RETURNING event_id, event_name, event_date, event_tickets, event_location;
    `;

    const values = [event_name, event_date, event_tickets, event_location];
 ...
 * Inserts a new ticket for an event into the Ticket table.
 */
const createTicket = async (event_id, ticket_availability, ticket_price, ticket_type) => {
    const ticketQuery = `
        INSERT INTO Ticket (event_id, ticket_availability, ticket_price, ticket_type)
        VALUES ($1, $2, $3, $4)
        RETURNING ticket_id, event_id, ticket_availability, ticket_price, ticket_type;
    `;

    const values = [event_id, ticket_availability, ticket_price, ticket_type];

    try {
        const result = await pool.query(ticketQuery, values);
        return result.rows[0];
    } catch (err) {
        throw err;
    }
